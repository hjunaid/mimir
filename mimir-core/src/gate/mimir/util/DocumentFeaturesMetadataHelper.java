 ...
 * 
 * $Id: DocumentFeaturesMetadataHelper.java 14241 2011-08-12 15:59:27Z
 * ian_roberts $
 */
 ...
 * not be saved in the index. The values thus saved can be retrieved at search
 * time by calling {@link QueryEngine#getDocumentMetadataField(int, String)}.
 */
public class DocumentFeaturesMetadataHelper implements DocumentMetadataHelper {
  /**
   * A map storing the correspondence between the GATE document feature name and
   * the metadata field name in the Mimir index.
   */
  protected Map<String, String> featureNameToFieldName;

  private static Logger logger = Logger
    .getLogger(DocumentFeaturesMetadataHelper.class);

  /**
   * Creates a new DocumentFeaturesMetadataHelper.
   * 
   * @param featureNameToFieldName
   *          a map storing the correspondence between the GATE document feature
   *          name and the metadata field name; keys are names of document
   *          features; values are names of metadata fields.
   */
  public DocumentFeaturesMetadataHelper(Map<String, String> featureNameToFieldName) {
   ...
   * 
   * @param featureNames
   *          an array of feature names. For each indexed document, the values
   *          for the features specified here are obtained and stored in the
   *          index, as document metadata fields with the same names as the GATE
   *          document features. If you need the names of the Mimir document
   *          metadata fields to be different from the GATE document features,
   *          then you should use the
   *          {@link #DocumentFeaturesMetadataHelper(Map)} variant.
   */
    ...
    for(Map.Entry<String, String> mapping : featureNameToFieldName.entrySet()) {
      ...
      if(value != null) {
        if(value instanceof Serializable) {
          documentData.putMetadataField(mapping.getValue(), 
            (Serializable)value);
