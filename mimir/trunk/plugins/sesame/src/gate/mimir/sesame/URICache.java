...
public class URICache {
	 ...
	 */
	private static final String SPARQL_URI_SUFFIX_INT = "^^<http://www.w3.org/2001/XMLSchema#int>";
	...
	/**
	 * A class for objects that store the values of the non-nominal features for
	 * a given annotation. These objects are also used as keys in a Map used for
	 * caching.
	 */
	protected class NonNominalFeatureValues {
		public NonNominalFeatureValues(Annotation annotation,
			...
			for (int i = 0; i < owner.getUriFeatureNames().length; i++) {
				...
				try {
					...
					logger.warn("Could not parse URI value. Problem was: "
							+ e.toString());
					uriValues[i] = null;
				}
				if (uriValues[i] != MIMIR_NULL_URI)
					hasNonNominalFeatures = true;
			}
		...
		public boolean equals(Object obj) {
			...
			if (!Arrays.equals(numericValues, other.numericValues))
				return false;
			if (!Arrays.equals(textValues, other.textValues))
				return false;
			if (!Arrays.equals(uriValues, other.uriValues))
				return false;
			return true;
		}
	...
	public URICache(SesameSemanticAnnotationHelper owner) {
		this.l1CacheSize = DEFAULT_L1_SIZE;
		this.l2CacheSize = DEFAULT_L2_SIZE;
		this.l3CacheSize = DEFAULT_L3_SIZE;
		this.owner = owner;
		this.annTemplateCache = new LinkedHashMap<ShortArray, URIAndMap>(
				l1CacheSize, DEFAULT_LOAD_FACTOR, true) {
			/*
			 * (non-Javadoc)
			 * 
			 * @see
		...
		this.mentionsCache = new LinkedHashMap<URIAndInt, URI>(l3CacheSize,
	...
	public double getOrdiL1CacheHitRatio() {
		if (ordiL1CacheHits == 0 && ordiL1CacheMisses == 0) {
			...
			return (double) ordiL1CacheHits
					/ (ordiL1CacheHits + ordiL1CacheMisses);
		}
	}
	...
	public double getOrdiL2CacheHitRatio() {
		if (ordiL2CacheHits == 0 && ordiL2CacheMisses == 0) {
			...
			return (double) ordiL2CacheHits
					/ (ordiL2CacheHits + ordiL2CacheMisses);
		}
	}
	...
	public double getOrdiL3CacheHitRatio() {
		if (ordiL3CacheHits == 0 && ordiL3CacheMisses == 0) {
			...
			return (double) ordiL3CacheHits
					/ (ordiL3CacheHits + ordiL3CacheMisses);
		}
	}
	...
	protected URI getFirstLevelATInstance(String[] nominalValues)
		...
		try {
			TupleQuery tq = owner.connection.prepareTupleQuery(
					QueryLanguage.SPARQL, query.toString(), MIMIR_NAMESPACE);
			TupleQueryResult result = tq.evaluate();
			if (result.hasNext()) {
				BindingSet binding = result.next();
				topLevelTemplateInstance = (URI) binding.getBinding(
						"annotationTemplateInstance").getValue();
			}
			if (topLevelTemplateInstance == null) {
				...
				String suffix = (UUID.randomUUID()).toString().replaceAll("-",
						"_");
				topLevelTemplateInstance = owner.factory.createURI(
						MIMIR_NAMESPACE, owner.getAnnotationType()
	...
	/**
	 * Finds a specialised instance of annotation template (i.e. an instance
	 * that has non-nominal properties set). If none exists in the knowledge
	 * base, one is created and then returned.
	 * 
	 * @param atInstance
	...
	protected URI getSpecialisedATInstance(URI atInstance,
		...
		try {
			...
			if (result.hasNext()) {
				BindingSet binding = result.next();
				specialisedInstance = (URI) binding
						.getBinding("specATInstance").getValue();
			}
			if (specialisedInstance == null) {
				...
				String suffix = (UUID.randomUUID()).toString().replaceAll("-",
						"_");
				specialisedInstance = owner.factory.createURI(MIMIR_NAMESPACE,
				...
				for (int i = 0; i < textValues.length; i++) {
					owner.connection.add(specialisedInstance,
							...
							owner.factory.createLiteral(textValues[i]),
							MIMIR_GRAPH_URI);
				}
				for (int i = 0; i < uriValues.length; i++) {
					owner.connection.add(specialisedInstance,
							owner.uriFeaturePredicates[i], uriValues[i],
							MIMIR_GRAPH_URI);
				}
				owner.connection.commit();
			} else {
	...
	/**
	 * Obtains the URIs for the mentions associated with the top level
	 * annotation template and the specialised annotation template (if one is
	 * needed) for a given annotation.
	 * 
	 * @param annotation
	...
	public URI[] getMentionURIs(Annotation annotation, int length) {
		...
		for (int i = 0; i < nominalValues.length; i++) {
			if (nominalValues[i] == MIMIR_NULL_STRING) {
				...
				short valueShort = nominalValuesToShort[i]
						.getShort(nominalValues[i]);
		...
		// build the key value
		ShortArray nomValuesKey = new ShortArray(key,
				Arrays.hashCode(nominalValues));
		...
		if (topLevel == null) {
			...
			topLevel.map = new LinkedHashMap<NonNominalFeatureValues, URI>(
					l2CacheSize, DEFAULT_LOAD_FACTOR, true) {
				@Override
				protected boolean removeEldestEntry(
		...
		if (nonNomvalues.hasNonNominalFeatures) {
			...
			if (specialisedInstance == null) {
				...
				l2CacheMisses++;
				try {
					specialisedInstance = getSpecialisedATInstance(
							topLevelTemplateInstance,
							nonNomvalues.numericValues,
							nonNomvalues.textValues, nonNomvalues.uriValues);
				} catch (RepositoryException e) {
					// TODO Auto-generated catch block
					e.printStackTrace();
				} catch (QueryEvaluationException e) {
		...
		URI firstLevelMention = null;
		try {
			firstLevelMention = getMention(topLevelTemplateInstance, length);
		} catch (RepositoryException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		} catch (QueryEvaluationException e) {
		...
		if (specialisedInstance != null) {
			URI secondLevelMention = null;
			try {
				secondLevelMention = getMention(specialisedInstance, length);
			} catch (RepositoryException e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
			} catch (QueryEvaluationException e) {
	 ...
	 * searches the knowledge base. If still not found, it creates the new
	 * mention URI and returns it.
	 * 
	 * @param atInstanceURI
	...
	protected URI getMention(URI atInstanceURI, int length)
		...
		if (mentionURI == null) {
			...
			query.append("}");
			try {
				TupleQuery tq = owner.connection
						.prepareTupleQuery(QueryLanguage.SPARQL,
								query.toString(), MIMIR_NAMESPACE);
				TupleQueryResult result = tq.evaluate();
				if (result.hasNext()) {
					BindingSet binding = result.next();
					mentionURI = (URI) binding.getBinding("mentionInstance")
							.getValue();
				}
				if (mentionURI == null) {
					// ordi L3 miss
					...
					String suffix = (UUID.randomUUID()).toString().replaceAll(
							"-", "_");
					mentionURI = owner.factory.createURI(MIMIR_NAMESPACE,
							atInstanceURI.getLocalName() + ":"
									+ (owner.mentionUniqueId++) + "_" + suffix); // TODO
					// place
					// suffix
					owner.connection.add(atInstanceURI, HAS_MENTION_PROP_URI,
							mentionURI, MIMIR_GRAPH_URI);
					owner.connection.add(mentionURI, HAS_LENGTH_PROP_URI,
							owner.factory.createLiteral(length),
							MIMIR_GRAPH_URI);
					owner.connection.commit();
				} else {
					ordiL3CacheHits++;
				...
				logger.error(
						"Malformed query exception, while trying to obtain "
								+ "the mention URI. The query sent to "
								+ "ORDI was :\n" + query.toString(), e);
			}
		} else {
			l3CacheHits++;
