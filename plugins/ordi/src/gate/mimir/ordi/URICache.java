/*
 * URICache.java Copyright (c) 1998-2009, The University of Sheffield.
 * 
 ...
 * 
 * Valentin Tablan, 18 Aug 2009
 * 
 * $Id$
 */
package gate.mimir.ordi;

import static gate.mimir.ordi.ORDISemanticAnnotationHelper.HAS_LENGTH_PROP_URI;
import static gate.mimir.ordi.ORDISemanticAnnotationHelper.HAS_MENTION_PROP_URI;
import static gate.mimir.ordi.ORDISemanticAnnotationHelper.HAS_SPECIALISATION_PROP_URI;
import static gate.mimir.ordi.ORDISemanticAnnotationHelper.MIMIR_GRAPH_URI;
import static gate.mimir.ordi.ORDISemanticAnnotationHelper.MIMIR_NULL_DOUBLE;
import static gate.mimir.ordi.ORDISemanticAnnotationHelper.MIMIR_NULL_STRING;
import static gate.mimir.ordi.ORDISemanticAnnotationHelper.MIMIR_NULL_URI;
...
public class URICache {
  ...
  protected class NonNominalFeatureValues {
    public NonNominalFeatureValues(Annotation annotation, TFactory tripleFactory) {
        ...
        if(value != null) {
          if(value instanceof Number) {
            ...
            try {
              ...
              logger
                      .warn("Value provided for feature \""
        ...
        try {
          uriValues[i] =
                  value == null || value.length() == 0
                          ? MIMIR_NULL_URI
                          : tripleFactory.createURI(value);
        } catch(Exception e) {
          logger
                  .warn("Could not parse URI value. Problem was: "
                          + e.toString());
          uriValues[i] = null;
        }
  ...
  public URICache(ORDISemanticAnnotationHelper owner) {
    this.l1CacheSize = DEFAULT_L1_SIZE;
    this.l2CacheSize = DEFAULT_L2_SIZE;
    this.l3CacheSize = DEFAULT_L3_SIZE;
    this.owner = owner;
    this.annTemplateCache =
            new LinkedHashMap<ShortArray, URIAndMap>(l1CacheSize,
              ...
              protected boolean removeEldestEntry(
                      Entry<ShortArray, URIAndMap> eldest) {
                return size() > URICache.this.l1CacheSize;
              }
            };
    this.mentionsCache =
            new LinkedHashMap<URIAndInt, URI>(l3CacheSize, DEFAULT_LOAD_FACTOR,
                    true) {
              /*
               * (non-Javadoc)
               * 
               ...
               */
              @Override
              protected boolean removeEldestEntry(Entry<URIAndInt, URI> eldest) {
                return size() > URICache.this.l3CacheSize;
              }
            };
    l1CacheHits = 0;
    l1CacheMisses = 0;
    ...
    ordiL3CacheHits = 0;
    ordiL3CacheMisses = 0;
    nominalValuesToShort =
            new Object2ShortMap[owner.getNominalFeatureNames().length];
    for(int i = 0; i < owner.getNominalFeatureNames().length; i++) {
      nominalValuesToShort[i] =
              new Object2ShortOpenHashMap<String>(l2CacheSize);
   ...
   */
  protected URI getMention(URI atInstanceURI, int length) throws ORDIException,
          IndexException {
    // first try the cache
    ...
    if(mentionURI == null) {
      ...
      try {
        tupleQuery =
                owner.parser.parseQuery(query.toString(), MIMIR_NAMESPACE)
                        .getTupleExpr();
      ...
      CloseableIterator<? extends BindingSet> result =
              owner.ordiConnection.evaluate(tupleQuery, bindings,
      ...
      if(mentionURI == null) {
        ...
        mentionURI =
                owner.ordiFactory.createURI(MIMIR_NAMESPACE, atInstanceURI
                        .getLocalName()
                        + ":"
                        + (owner.mentionUniqueId++)
                        + "_"
                        + suffix); // TODO place suffix
        owner.ordiConnection.addStatement(atInstanceURI, HAS_MENTION_PROP_URI,
                mentionURI, MIMIR_GRAPH_URI);
        owner.ordiConnection.addStatement(mentionURI, HAS_LENGTH_PROP_URI,
                owner.ordiFactory.createLiteral(length), MIMIR_GRAPH_URI);
        owner.ordiConnection.commit();
      } else {
        ordiL3CacheHits++;
      }
  ...
  public URI[] getMentionURIs(Annotation annotation, int length)
          throws ORDIException, IndexException {
    ...
    for(int i = 0; i < owner.getNominalFeatureNames().length; i++) {
      Object value = annotation.getFeatures().get(owner.getNominalFeatureNames()[i]);
      if(value == null) {
    ...
    NonNominalFeatureValues nonNomvalues =
            new NonNominalFeatureValues(annotation, owner.ordiFactory);
    ...
    if(specialisedInstance != null) {
      ...
      if(secondLevelMention == null) {
        logger
                .error("Could not find or create mention for specialised annotation template "
                        + specialisedInstance);
      }
  ...
  protected URI getFirstLevelATInstance(String[] nominalValues)
    ...
    if(result.hasNext()) {
      BindingSet binding = result.next();
      topLevelTemplateInstance =
              (URI)binding.getBinding("annotationTemplateInstance").getValue();
    }
    if(topLevelTemplateInstance == null) {
      ...
      String suffix = (UUID.randomUUID()).toString().replaceAll("-", "_");
      topLevelTemplateInstance =
              owner.ordiFactory.createURI(MIMIR_NAMESPACE, owner.getAnnotationType()
                      + (owner.atInstanceUniqueId++) + "_"
                      + suffix); // TODO place suffix
      owner.ordiConnection.addStatement(topLevelTemplateInstance, RDF.TYPE,
              owner.annotationTemplateURILevel1, MIMIR_GRAPH_URI);
      for(int i = 0; i < owner.nominalFeaturePredicates.length; i++) {
        owner.ordiConnection.addStatement(topLevelTemplateInstance,
                owner.nominalFeaturePredicates[i], owner.ordiFactory
                        .createLiteral(nominalValues[i]), MIMIR_GRAPH_URI);
      }
      owner.ordiConnection.commit();
    } else {
      ordiL1CacheHits++;
   ...
   *         exists.
   * @throws ORDIException
   *           if there are problems accessing ORDI.
   * @throws IndexException
   */
  protected URI getSpecialisedATInstance(URI atInstance,
    ...
    if(specialisedInstance == null) {
      ...
      String suffix = (UUID.randomUUID()).toString().replaceAll("-", "_");
      specialisedInstance =
              owner.ordiFactory.createURI(MIMIR_NAMESPACE, atInstance
                      .getLocalName()
                      + "_"
                      + (owner.atInstanceSpecUniqueId++)
                      + "_"
                      + suffix); // TODO place suffix
      owner.ordiConnection.addStatement(specialisedInstance, RDF.TYPE,
              owner.annotationTemplateURILevel2, MIMIR_GRAPH_URI);
      owner.ordiConnection
              .addStatement(atInstance, HAS_SPECIALISATION_PROP_URI,
                      specialisedInstance, MIMIR_GRAPH_URI);
      for(int i = 0; i < numericValues.length; i++) {
        owner.ordiConnection.addStatement(specialisedInstance,
                owner.floatFeaturePredicates[i], owner.ordiFactory
                        .createLiteral(numericValues[i]), MIMIR_GRAPH_URI);
      }
      for(int i = 0; i < textValues.length; i++) {
        owner.ordiConnection.addStatement(specialisedInstance,
                owner.textFeaturePredicates[i], owner.ordiFactory
                        .createLiteral(textValues[i]), MIMIR_GRAPH_URI);
      }
      for(int i = 0; i < uriValues.length; i++) {
        owner.ordiConnection.addStatement(specialisedInstance,
   ...
   */
  protected ORDISemanticAnnotationHelper owner;
