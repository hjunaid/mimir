...
public class SesameSemanticAnnotationHelper extends
  ...
  public void init(Indexer indexer) {
    ...
    try {
      ...
      if(!staticInitDone) staticInit(connection, factory);
  ...
  public void close(Indexer indexer) {
    if(initDone) {
      ...
      try {
        ...
        logger.error("Error while closing Sesame Repository Connection", e);
      }
      connection = null;
      parser = null;
      initDone = false;
      ...
      int clientCount =
              (Integer)indexer.getIndexConfig().getContext()
                      .get(SESAME_CONNECTION_COUNT_KEY);
      clientCount--;
      indexer.getIndexConfig().getContext()
              .put(SESAME_CONNECTION_COUNT_KEY, Integer.valueOf(clientCount));
      if(clientCount == 0) {
        // shutting down the ORDI Source
        RepositoryManager manager =
                (RepositoryManager)indexer.getIndexConfig().getContext()
                        .get(SESAME_RMANAGER_KEY);
        if(manager != null) {
          logger.info("Shutting down Sesame repositoryManager");
          manager.shutDown();
          indexer.getIndexConfig().getContext().remove(SESAME_RMANAGER_KEY);
        }
      }
    }
  }
  ...
  public void close(QueryEngine qEngine) {
    if(initDone) {
      ...
      // if we're the last helper, shutdown the ORDI source
      int clientCount =
              (Integer)qEngine.getIndexConfig().getContext()
                      .get(SESAME_CONNECTION_COUNT_KEY);
      clientCount--;
      ...
      if(clientCount == 0) {
        // shutting down the ORDI Source
        RepositoryManager manager =
                (RepositoryManager)qEngine.getIndexConfig().getContext()
                        .get(SESAME_RMANAGER_KEY);
        if(manager != null) {
          logger.info("Shutting down Sesame repositoryManager");
          manager.shutDown();
          qEngine.getIndexConfig().getContext().remove(SESAME_RMANAGER_KEY);
        }
      }
    }
  }
  ...
  public RepositoryConnection getRepositoryConnection(IndexConfig config)
    ...
    RepositoryManager manager =
            (RepositoryManager)config.getContext().get(SESAME_RMANAGER_KEY);
    String repositoryName =
            (String)config.getContext().get(SESAME_REPOSITORY_NAME_KEY);
    ...
    if(manager == null) {
      ...
      int clientCount =
              (Integer)config.getContext().get(SESAME_CONNECTION_COUNT_KEY);
    ...
    try {
      try {
        ...
        try {
          ...
          logger.warn("Can not parse repository configuration path", rpe);
        }
      } catch(IOException ioe) {
        logger.warn("Can not instantiate repository configuration graph", ioe);
      }
    } catch(RDFHandlerException rhe) {
      logger.warn("Internal graph initialization exception!", rhe);
    }
    Iterator<Statement> iterSt =
  ...
  protected static void staticInit(RepositoryConnection connection,
    ...
    ANNOTATION_TEMPLATE_LEVEL1_CLASS_URI =
            factory.createURI(MIMIR_NAMESPACE, "AnnotationTemplateL1");
    createClass(ANNOTATION_TEMPLATE_LEVEL1_CLASS_URI, connection, factory);
    ANNOTATION_TEMPLATE_LEVEL2_CLASS_URI =
            factory.createURI(MIMIR_NAMESPACE, "AnnotationTemplateL2");
    ...
    HAS_SPECIALISATION_PROP_URI =
            factory.createURI(MIMIR_NAMESPACE, "hasSpecialisation");
    connection.add(HAS_SPECIALISATION_PROP_URI, RDF.TYPE, OWL.OBJECTPROPERTY,
            MIMIR_GRAPH_URI);
  ...
  protected void initCommon() throws RepositoryException {
    ...
    annotationTemplateURILevel1 =
            factory.createURI(MIMIR_NAMESPACE, annotationType + "L1");
    ...
    annotationTemplateURILevel2 =
            factory.createURI(MIMIR_NAMESPACE, annotationType + "L2");
    ...
    for(int i = 0; i < nominalFeatureNames.length; i++) {
      nominalFeaturePredicates[i] =
              factory.createURI(MIMIR_NAMESPACE, annotationType + "_has"
    ...
    for(int i = 0; i < textFeatureNames.length; i++) {
      textFeaturePredicates[i] =
              factory.createURI(MIMIR_NAMESPACE, annotationType + "+has"
                      + textFeatureNames[i]);
    ...
    if(Arrays.asList(uriFeatureNames).contains(SEMANTIC_CONSTRAINT_FEATURE)) {
      ...
      uriFeatureNamesPlusSemanticConstraint =
              new String[uriFeatureNames.length + 1];
      System.arraycopy(uriFeatureNames, 0,
              uriFeatureNamesPlusSemanticConstraint, 0, uriFeatureNames.length);
  ...
  protected static boolean createClass(URI classURI,
    ...
    RepositoryResult<Statement> result =
            connection.getStatements(classURI, RDF.TYPE, OWL.CLASS, false,
                    MIMIR_GRAPH_URI);
    if(result.hasNext()) {
  ...
  protected static boolean createDatatypeProperty(URI propertyURI,
    ...
    RepositoryResult<Statement> result =
            connection.getStatements(propertyURI, RDF.TYPE,
                    OWL.DATATYPEPROPERTY, false, MIMIR_GRAPH_URI);
    if(result.hasNext()) {
    ...
    Statement statement =
            new StatementImpl(propertyURI, RDF.TYPE, OWL.DATATYPEPROPERTY);
    connection.add(statement, MIMIR_GRAPH_URI);
    connection.add(propertyURI, RDFS.RANGE, propertyRange, MIMIR_GRAPH_URI);
    connection.add(propertyURI, RDFS.DOMAIN, propertyDomain, MIMIR_GRAPH_URI);
    return statement != null;
  }
  ...
  protected static boolean createRDFProperty(URI propertyURI,
    ...
    // check if the property exists already
    RepositoryResult<Statement> result =
            connection.getStatements(propertyURI, RDF.TYPE, RDF.PROPERTY,
                    false, MIMIR_GRAPH_URI);
    if(result.hasNext()) {
      // property already exists
      return false;
    }
    // else, create new property
    Statement statement =
            new StatementImpl(propertyURI, RDF.TYPE, RDF.PROPERTY);
    connection.add(statement, MIMIR_GRAPH_URI);
    return statement != null;
  }
